...
## Vấn đề thực tế
Tìm kiếm bài viết hiện tại dùng câu lệnh `LIKE '%' || :text || '%'` quét toàn bộ bảng trên cả trường `fullContent` khổng lồ. Với cơ sở dữ liệu hàng ngàn bài viết, tìm kiếm có thể mất vài giây và giật lag nghiêm trọng khi gõ từng ký tự.
...
```
Bạn đang thực hiện task [ENH-02] "Tích hợp Room Full-Text Search (FTS4 / FTS5) cho Tìm Kiếm Bài Báo" trong repo RSS Cat Hub (com.mckimquyen.reader, xem CLAUDE.md để hiểu kiến trúc). Đọc kỹ "Vấn đề thực tế" + "Acceptance Criteria" trong file doc/task/todo/ENH-02_room-fts-search.md trước khi bắt đầu.
...
3. Với thay đổi kiến trúc/thiết kế quan trọng (không bắt buộc cho fix nhỏ, 1-2 dòng), tham khảo ý kiến độc lập từ 2 AI agent khác trước khi chốt:
   - `codex exec -s workspace-write "Review approach cho task [ENH-02] trong repo RSS Cat Hub: <tóm tắt ngắn cách bạn định làm>. Chỉ ra rủi ro/cách tốt hơn nếu có."`
   - `claude -p "Review approach cho task [ENH-02] trong repo RSS Cat Hub: <tóm tắt ngắn cách bạn định làm>. Chỉ ra rủi ro/cách tốt hơn nếu có." --allowedTools "Read Grep Glob"`

...
## Vấn đề thực tế
Khi đọc được một câu nói hay hoặc một đoạn phân tích sâu sắc, người dùng thường có thói quen chụp màn hình để đăng lên Facebook, X (Twitter), Threads, LinkedIn. Hiện tại app không có chức năng tạo thẻ trích dẫn chuyên dụng, người dùng phải chụp màn hình thô.
...
```
Bạn đang thực hiện task [IDEA-03] "Trình Tạo Ảnh Trích Dẫn "Quote Card" Chia Sẻ Mạng Xã Hội" trong repo RSS Cat Hub (com.mckimquyen.reader, xem CLAUDE.md để hiểu kiến trúc). Đọc kỹ "Vấn đề thực tế" + "Acceptance Criteria" trong file doc/task/todo/IDEA-03_quote-card-social-share.md trước khi bắt đầu.
...
3. Với thay đổi kiến trúc/thiết kế quan trọng (không bắt buộc cho fix nhỏ, 1-2 dòng), tham khảo ý kiến độc lập từ 2 AI agent khác trước khi chốt:
   - `codex exec -s workspace-write "Review approach cho task [IDEA-03] trong repo RSS Cat Hub: <tóm tắt ngắn cách bạn định làm>. Chỉ ra rủi ro/cách tốt hơn nếu có."`
   - `claude -p "Review approach cho task [IDEA-03] trong repo RSS Cat Hub: <tóm tắt ngắn cách bạn định làm>. Chỉ ra rủi ro/cách tốt hơn nếu có." --allowedTools "Read Grep Glob"`
